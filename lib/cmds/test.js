...
mods = {
    init: function(options) {
        ...
        if (options.parsed.downstream) {
            this.modules = this.getDownstreamComponents(module);
        }
    ...
    cli: function(port, callback) {
        ...
        if (util.isYUI() && (this.module.name === 'yui3' || this.modules)) {
            ...
            batch.forEach(function(mod) {
                ...
                if (self.options.parsed.downstream && self.modules.indexOf(mod) === -1) {
                    return;
                }
    ...
    resolveTests: function(excludes) {
        ...
        if (util.exists(testPath)) {
            if (this.options.parsed.downstream) {
                this.modules.forEach(function (mod) {
                    if (exclude[mod]) {
                        return;
                    }
                    var testPath = path.resolve(self.module.dir, '../', mod, 'tests', 'unit'),
                        newTests = util.getTests(testPath, self.coverage);
    ...
    getDownstreamComponents: function (module) {
        var Depcalc = require('depcalc'),
            yuipath = path.resolve(git.findRoot(), '../'),
            YUIConfig = {
                componentSource: true,
                moduleMap: require(path.resolve(yuipath, 'src/loader/js/yui3.json')),
                componentMap: Depcalc.utilities.generateComponentMap(path.resolve(yuipath, 'src/*/build.json'))
            },
            calculator = new Depcalc(YUIConfig),
            depTree = calculator.resolve(module.name);

        return depTree.downstream.components;
    },
    help: function() {
        return [
            'test',
